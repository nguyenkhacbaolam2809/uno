...
MenuView::Button MenuView::makeButton(int x, int y, int w, int h, const std::string & text, Color color)
{
    ...
    btn.text = text;
    btn.color = color;
    btn.hoverColor = Fade(color, 0.7f);
    btn.hovered = false;
    return btn;
}
...
MenuResult MenuView::showMainMenu()
{
    while (!WindowShouldClose())
    {
        ...
        drawTitle(msg(1), 60, 80, GOLD);
        drawTextCentered("Multiplayer Card Game", 140, 24, WHITE_SMOKE);

        int bw = 300, bh = 50, bx = (SCREEN_W - bw) / 2, by = 200, gap = 15;

        std::vector<Button> btns;
        btns.push_back(makeButton(bx, by, bw, bh, msg(2), (Color){ 237, 28, 36, 255 }));
        btns.push_back(makeButton(bx, by + (bh + gap) * 1, bw, bh, msg(3), (Color){ 0, 114, 188, 255 }));
        btns.push_back(makeButton(bx, by + (bh + gap) * 2, bw, bh, msg(4), (Color){ 0, 155, 72, 255 }));
        btns.push_back(makeButton(bx, by + (bh + gap) * 3, bw, bh, msg(5), (Color){ 180, 100, 20, 255 }));
        btns.push_back(makeButton(bx, by + (bh + gap) * 4, bw, bh, msg(6), (Color){ 80, 80, 80, 255 }));
...
MenuResult MenuView::showDifficultySelect()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        diffBtns.push_back(makeButton((SCREEN_W - bw) / 2, startY + 40, bw, bh, msg(8), (Color){ 0, 155, 72, 255 }));
        diffBtns.push_back(makeButton((SCREEN_W - bw) / 2, startY + 40 + (bh + gap), bw, bh, msg(9), (Color){ 180, 140, 20, 255 }));
        diffBtns.push_back(makeButton((SCREEN_W - bw) / 2, startY + 40 + (bh + gap) * 2, bw, bh, msg(10), (Color){ 237, 28, 36, 255 }));
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (i == diffChoice - 1)
            {
                DrawRectangleRoundedLines(diffBtns[i].rect, 0.3f, 10, 3, GOLD);
            }
        }
        ...
        DrawRectangleRounded(nameRect, 0.3f, 10, (Color){ 60, 60, 70, 255 });
        if (editingName)
            DrawRectangleRoundedLines(nameRect, 0.3f, 10, 2, GOLD);
        ...
        DrawRectangleRounded(minusRect2, 0.3f, 10, (Color){ 200, 60, 60, 255 });
        DrawRectangleRounded(plusRect2, 0.3f, 10, (Color){ 60, 200, 60, 255 });
        ...
        Color vietCol = result.vietRules ? (Color){ 0, 200, 100, 255 } : (Color){ 80, 80, 90, 255 };
        ...
        DrawRectangleRounded(startBtn, 0.3f, 10, (Color){ 0, 155, 72, 255 });
...
MenuResult MenuView::showLocalSetup()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        DrawRectangleRounded(minusRect, 0.3f, 10, (Color){ 200, 60, 60, 255 });
        DrawRectangleRounded(plusRect, 0.3f, 10, (Color){ 60, 200, 60, 255 });
...
MenuResult MenuView::showMixedSetup()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        DrawRectangleRounded(hMinus, 0.3f, 10, (Color){ 200, 60, 60, 255 });
        DrawRectangleRounded(hPlus, 0.3f, 10, (Color){ 60, 200, 60, 255 });
        DrawRectangleRounded(bMinus, 0.3f, 10, (Color){ 200, 60, 60, 255 });
        DrawRectangleRounded(bPlus, 0.3f, 10, (Color){ 60, 200, 60, 255 });
        ...
        diffBtns.push_back(makeButton(startX, 310, bw, bh, msg(8), (Color){ 0, 155, 72, 255 }));
        diffBtns.push_back(makeButton(startX, 310 + (bh + gap), bw, bh, msg(9), (Color){ 180, 140, 20, 255 }));
        diffBtns.push_back(makeButton(startX, 310 + (bh + gap) * 2, bw, bh, msg(10), (Color){ 237, 28, 36, 255 }));
...
MenuResult MenuView::showLanMenu()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        drawTitle(msg(5), 40, 48, GOLD);

        int bw = 260, bh = 50, gap = 20;
        int bx = (SCREEN_W - bw) / 2;
        int by = 160;

        Button hostBtn = makeButton(bx, by, bw, bh, msg(13), (Color){ 0, 114, 188, 255 });
        Button joinBtn = makeButton(bx, by + bh + gap, bw, bh, msg(14), (Color){ 0, 155, 72, 255 });
        ...
        DrawRectangleRounded(ipRect, 0.3f, 10, (Color){ 60, 60, 70, 255 });
        if (editingIp) DrawRectangleRoundedLines(ipRect, 0.3f, 10, 2, GOLD);
        ...
        DrawRectangleRounded(portRect, 0.3f, 10, (Color){ 60, 60, 70, 255 });
        if (editingPort) DrawRectangleRoundedLines(portRect, 0.3f, 10, 2, GOLD);
